...
export default function FeedListSave(){
  let FeedListSaveQuery = useInfiniteQuery('SavedFeed', apiClient.getSavedFeedList, {getNextPageParam: (lastPage, pages) => {return pages.length + 1}})
    ...
    return (
      <Fragment>
          ...
          <div style={{display:'flex', justifyContent:'center', pb:7}}>
            <Link href=""></Link>
            <Button variant='outlined' color='primary' size='large' onClick={FeedListSaveQuery.fetchNextPage}>더보기</Button> 
          </div>
      </Fragment>
    )
  }
