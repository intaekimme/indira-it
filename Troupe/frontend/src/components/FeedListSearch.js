...
export default function FeedListSearch(props){
  let FeedListSearchQuery = useInfiniteQuery(['tagSearch'], () => apiClient.feedTagSearch({pageParam: 0, tags: props.tags}), {retry:true, getNextPageParam: (lastPage, pages) => {return 1}})
    ...
    return (
        <Fragment>
          ...
          <div style={{display:'flex', justifyContent:'center', pb:7}}>
            <Link href=""></Link>
            <Button variant='outlined' color='primary' size='large' onClick={FeedListSearchQuery.fetchNextPage}>더보기</Button> 
          </div>
        </Fragment>
    )
  }
